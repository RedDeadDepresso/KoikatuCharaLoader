...
def test_load_amanatsu_character(data_dir):
...
def test_save_amanatsu_character(data_dir):
    with open(data_dir / "al_chara.png", "rb") as f:
        raw_data = f.read()
...
def test_json_amanatsu(data_dir):
    al = AmanatsuCharaData.load(data_dir / "al_chara.png")
    tmpfile = tempfile.NamedTemporaryFile()
    al.save_json(tmpfile.name)


def test_repr_amanatsu(data_dir):
    al = AmanatsuCharaData.load(data_dir / "al_chara.png")
    repr_text = repr(al)
...
def test_save_al_coordinate(data_dir):
    from kkloader.AmanatsuCharaData import CoordinateEntry
